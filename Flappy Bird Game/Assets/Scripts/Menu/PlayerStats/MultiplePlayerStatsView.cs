...
public class MultiplePlayerStatsView : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler
{
	...
	private void Start()
	{
		_playerNameLabelPos = new Vector3(640, 570, 0);			                    // tymi danymi nalezy sterowac podczas ustawiania pozycji Name, Highscore, Achievements
		_highscoreLabelPos = new Vector3(960, 570, 0);
		_achievementsLabelPos = new Vector3(1280, 570, 0);
	...
	private void FillContainersOnStart()										// wypełnia tyle kontenerów, ile znajduje się w hierarchii
	{
		for (int i = 0; i < _scope; i++)
		{
			...
			_playerNameLabelPos.y -= 80;
			_highscoreLabelPos.y -= 80;
			_achievementsLabelPos.y -= 80;
		}
	}
	...
	private void ScrollDataFilledContainersDown(int index)					  // delta ujemna = scrolluje w dół
	{
        ...
        int upperBorder = 200;      // przesuwanie kontenerów zatrzyma się, kiedy pozycja ostatniego kontenera bedzie mniejsza niz 200

        // dopoki ostatni kontener nie zawiera ostatniego elementu listy playerów oraz Y-pozycja ostatniego kontenera jest wieksza niz 200
        if (!(_listOfContainers[_scope - 1].PlayerName.text.ToString().Equals(_dataToDisplay.EntireList[_dataToDisplay.EntireList.Count - 1].PlayerName) && _listOfContainers[_scope - 1].PlayerName.transform.position.y > upperBorder))
        {
			...
			if (_currentTopEntry < (_dataToDisplay.EntireList.Count - _scope) && _listOfContainers[0].AchievementSingleEntryViewInstance.Complete10Inactive.transform.position.y > 650)
			{
				for (int i = 0; i < _scope; i++)
				{
	...
	private void ScrollDataFilledContainersUp(int index)                      // delta dodatnia = scrolluje w gore
	{
        ...
        int lowerBorder;
        if (_scope == 6)
            lowerBorder = 130;        // przesuwanie kontenerów zatrzyma się, kiedy pozycja SZOSTEGO kontenera bedzie wieksza 130
        else
            lowerBorder = 50;        // przesuwanie kontenerów zatrzyma się, kiedy pozycja ostatniego kontenera bedzie wieksza 50
        ...
        if (!(_listOfContainers[0].PlayerName.text.ToString().Equals(_dataToDisplay.EntireList[0].PlayerName) && _listOfContainers[_scope - 1].PlayerName.transform.position.y < lowerBorder))
        {
            ...
            if (_currentTopEntry > 0 && _listOfContainers[0].AchievementSingleEntryViewInstance.Complete10Inactive.transform.position.y < 570)
            {
                _currentTopEntry--;
                ReplaceProfiles(_currentTopEntry);
                for (int i = 0; i < _scope; i++)
